		   ...
		   MPI_Comm comm_col )
{
  ...
  */
  ...
  // broadcast within rows
	...
	send_buffa = ( double * ) malloc (sizeof ( double ) * (local_m * BK ));
        send_buffb = ( double * ) malloc (sizeof ( double ) * (local_n * BK ));
	
	// recieve buffer
	recv_buffa = ( double * ) malloc (sizeof ( double ) * (local_m * BK ));
        recv_buffb = ( double * ) malloc (sizeof ( double ) * (local_n * BK ));

	for(i = 0; i < local_m * BK; i++){
		send_buffa[i] = 0;
		recv_buffa[i] = 0;
	}

	for(i = 0; i < local_n * BK; i++){
		send_buffb[i] = 0;
		recv_buffb[i] = 0;
	}
	...
	for(p = pp; p < pp + BK; p++ ){
	  ...
	  if ( curcol == mycol ){
		local_p_A = p / ncols;
		for ( i=0; i<local_m; i++ )
		  send_buffa[ (p - pp) * local_m + i ] = LOCAL_A( i, local_p_A );
	  }
	...
	for(p = pp; p < pp + BK; p++ ){
	  ...
	  if ( currow == myrow ){
		local_p_B = p / nrows;
		for ( j=0; j<local_n; j++ )
		  send_buffb[ (p - pp) * local_n + j ] = LOCAL_B( local_p_B , j );
	  }
	...
	MPI_Allreduce ( send_buffa, recv_buffa, (local_m * BK), MPI_DOUBLE, MPI_SUM, comm_row );
        MPI_Allreduce ( send_buffb, recv_buffb, (local_n * BK), MPI_DOUBLE, MPI_SUM, comm_col );
	
	dgemm_( "N", "T", &local_m, &local_n, &local_k, 
	&d_one, recv_buffa, &local_m, recv_buffb, 
